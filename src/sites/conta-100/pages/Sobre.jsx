  ...
  return (
    <>
      ...
      <section className="relative overflow-hidden bg-[#F4F8F6] px-6 pt-36 pb-16">
        <div className="pointer-events-none absolute -right-40 -top-24 h-[30rem] w-[30rem] rounded-full bg-[#123C66]/10 blur-3xl" />
      ...
      <section className="bg-[#FFFFFF] py-20">
        <div className="mx-auto grid max-w-7xl items-center gap-14 px-6 lg:grid-cols-2">
          <Reveal>
            <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-[#0E2A23] to-[#123C66] p-10">
              <div className="pointer-events-none absolute -right-12 -top-12 h-48 w-48 rounded-full bg-[#1F9D6B]/20 blur-2xl" />
              <span className="font-jakarta text-7xl font-extrabold text-[#1F9D6B]">{brand.since}</span>
              ...
              <div className="mt-8 flex items-center gap-4 border-t border-white/10 pt-6">
                <span className="grid h-12 w-12 place-items-center rounded-xl bg-[#1F9D6B] font-jakarta text-lg font-extrabold text-[#FFFFFF]">
                  BS
                </span>
                <div>
          ...
          <div className="mt-14 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {trustPoints.map((t, i) => (
              <Reveal key={t.title} delay={(i % 4) * 80}>
                <div className="flex h-full flex-col rounded-2xl bg-[#FFFFFF] p-7 ring-1 ring-[#0E2A23]/5">
                  ...
                  <h3 className="mt-5 font-jakarta text-lg font-bold text-[#0E2A23]">{t.title}</h3>
        ...
        <div className="mx-auto max-w-7xl px-6">
          ...
          <div className="mt-14 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {team.map((m, i) => (
              <Reveal key={m.name} delay={(i % 3) * 80}>
                <div className="flex h-full flex-col rounded-2xl border border-[#0E2A23]/10 bg-[#F4F8F6] p-8">
                  <span className="grid h-16 w-16 place-items-center rounded-2xl bg-[#0E2A23] font-jakarta text-2xl font-extrabold text-[#1F9D6B]">
                    {m.initials}
                  </span>
                  <h3 className="mt-5 font-jakarta text-xl font-bold text-[#0E2A23]">{m.name}</h3>
                  <p className="text-sm font-bold uppercase tracking-wide text-[#1F9D6B]">{m.role}</p>
          ...
          <Reveal delay={120}>
            <p className="mt-8 inline-flex items-center gap-2 text-sm text-[#51635C]/80">
              <Users className="h-4 w-4 text-[#1F9D6B]" /> Equipa pequena e próxima, entre 2 e 10 pessoas.
            </p>
          </Reveal>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-7xl px-6">
          ...
          <div className="mt-14 grid gap-6 md:grid-cols-3">
            {values.map((v, i) => (
              <Reveal key={v.title} delay={i * 90}>
                <div className="h-full rounded-2xl bg-[#FFFFFF] p-8 ring-1 ring-[#0E2A23]/5">
                  <span className="inline-grid h-12 w-12 place-items-center rounded-xl bg-[#123C66]/10 text-[#123C66]">
                    <v.icon className="h-6 w-6" strokeWidth={1.8} />
                  </span>
                  <h3 className="mt-5 font-jakarta text-xl font-bold text-[#0E2A23]">{v.title}</h3>
          ...
          <Reveal delay={120} className="mt-14 text-center">
            <Link
              ...
              className="inline-flex items-center gap-2 rounded-full bg-[#1F9D6B] px-8 py-4 font-sans text-base font-bold text-[#FFFFFF] transition-colors hover:bg-[#178a5c] cursor-pointer"
            >
