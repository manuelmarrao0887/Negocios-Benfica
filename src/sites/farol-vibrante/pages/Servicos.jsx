  ...
  return (
    <>
      ...
      <section className="relative overflow-hidden bg-[#F7F8FA] px-6 pt-36 pb-16">
        <GridLines />
        <div className="pointer-events-none absolute -right-20 -top-20 h-80 w-80 rounded-full bg-[#F2A33C]/15 blur-3xl" aria-hidden="true" />
        ...
        <div className="mx-auto max-w-7xl space-y-6 px-6">
          {services.map((s, i) => (
            <Reveal key={s.title} delay={(i % 3) * 70}>
              <div className="grid gap-8 rounded-2xl border border-[#E2E6EA] bg-[#FFFFFF] p-8 shadow-sm shadow-[#14181D]/[0.03] sm:p-10 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
                <div>
                  <div className="flex items-center gap-4">
                    ...
                    <span className="font-sora text-sm font-bold uppercase tracking-wider text-[#64727F]">
                      0{i + 1}
                    </span>
                  </div>
                  <h2 className="mt-5 font-sora text-2xl font-bold text-[#14181D] sm:text-3xl">{s.title}</h2>
                  <p className="mt-3 text-[15px] leading-relaxed text-[#4A5763]">{s.desc}</p>
                </div>
                <ul className="grid gap-3 sm:grid-cols-2">
                  {s.points.map((p) => (
                    <li
                      key={p}
                      className="flex items-start gap-3 rounded-xl border border-[#E2E6EA] bg-[#F7F8FA] p-4 text-sm leading-relaxed text-[#4A5763]"
                    >
                      <span className="mt-0.5 grid h-5 w-5 shrink-0 place-items-center rounded bg-[#F2A33C]/15 text-[#945600]">
          ...
          <div className="mt-12 grid gap-5 md:grid-cols-3">
            {sectors.map((s, i) => (
              <Reveal key={s.title} delay={i * 80}>
                <div className="group h-full rounded-2xl border border-[#E2E6EA] bg-[#FFFFFF] p-8 shadow-sm shadow-[#14181D]/[0.03] transition-colors duration-300 hover:border-[#F2A33C]/60">
          ...
          <div className="mt-14 grid gap-5 sm:grid-cols-2 lg:grid-cols-5">
            {process.map((p, i) => (
              <Reveal key={p.num} delay={(i % 5) * 70}>
                <div className="group relative h-full rounded-2xl border border-[#E2E6EA] bg-[#FFFFFF] p-6 shadow-sm shadow-[#14181D]/[0.03] transition-colors duration-300 hover:border-[#F2A33C]/60">
                  <span className="font-sora text-3xl font-bold text-[#F2A33C]/45 transition-colors group-hover:text-[#945600]">
                    {p.num}
                  </span>
                  <span className="mt-4 inline-grid h-11 w-11 place-items-center rounded-lg bg-[#F2A33C]/12 text-[#945600]">
                    <p.icon className="h-5 w-5" strokeWidth={1.8} />
                  </span>
                  <h3 className="mt-4 font-sora text-lg font-bold text-[#14181D]">{p.title}</h3>
                  <p className="mt-2 text-sm leading-relaxed text-[#4A5763]">{p.text}</p>
                </div>
              </Reveal>
            ))}
          </div>
      ...
      <section className="relative overflow-hidden bg-[#F7F8FA] py-20">
        ...
        <div className="relative mx-auto max-w-7xl px-6">
          <Reveal>
            <div className="grid overflow-hidden rounded-3xl border border-[#E2E6EA] shadow-sm shadow-[#14181D]/[0.04] lg:grid-cols-2">
              <div className="relative min-h-[280px]">
                ...
                />
                <div className="absolute inset-0 bg-gradient-to-r from-[#14181D]/20 to-[#F2A33C]/15 mix-blend-multiply" />
                <div className="absolute inset-0 bg-gradient-to-t from-[#14181D]/50 to-transparent lg:bg-gradient-to-r" />
              </div>
              <div className="flex flex-col justify-center bg-[#FFFFFF] p-8 sm:p-12">
                <h2 className="font-sora text-3xl font-bold leading-tight text-[#14181D] sm:text-4xl">
