  ...
  return (
    <>
                ...
                <div
                  className={
                    ...
                    (s.featured
                      ...
                      : 'border-[#E3E9F2] bg-white hover:border-[#2E7DF6]/30')
                  }
                >
                  <div className="flex items-center justify-between">
                    <span className="inline-grid h-14 w-14 place-items-center rounded-2xl bg-[#EAF1FE] text-[#2E7DF6] ring-1 ring-[#2E7DF6]/15">
                  ...
                  <h2 className="mt-6 font-space text-2xl font-bold text-[#0F1B2D]">{s.title}</h2>
                  <p className="mt-3 flex-1 text-[15px] leading-relaxed text-[#51607A]">{s.desc}</p>
                  <ul className="mt-6 space-y-2.5 border-t border-[#E3E9F2] pt-6">
                    {s.points.map((pt) => (
                      <li key={pt} className="flex items-center gap-2.5 text-sm text-[#475569]">
      ...
      <section className="bg-white py-24">
        <div className="mx-auto max-w-7xl px-6">
          <Reveal>
            <div className="relative overflow-hidden rounded-3xl border border-[#2E7DF6]/25 bg-gradient-to-br from-[#F2F7FF] to-white px-8 py-12 sm:px-14 sm:py-16">
              ...
              <div className="relative grid items-center gap-10 lg:grid-cols-[1.1fr_0.9fr]">
                <div>
                  <span className="inline-flex items-center gap-2 rounded-full bg-[#2E7DF6]/15 px-4 py-1.5 font-sans text-xs font-bold uppercase tracking-wider text-[#1A5FCC]">
                    <Lightbulb className="h-4 w-4" /> A nossa especialidade
                  </span>
                  <h2 className="mt-5 font-space text-3xl font-bold leading-tight text-[#0F1B2D] sm:text-4xl">
                  ...
                  <p className="mt-4 text-lg leading-relaxed text-[#51607A]">
                    ...
                    acabamento testado e <span className="text-[#0F1B2D]">2 anos de garantia</span>.
                  </p>
                  <a
                ...
                <div className="grid grid-cols-2 gap-4">
                  {[
                    ...
                    <div key={c.l} className="rounded-2xl border border-[#E3E9F2] bg-white p-5 text-center">
                      <div className="font-space text-2xl font-bold text-[#0B8C73]">{c.v}</div>
                      <div className="mt-1 text-xs font-semibold uppercase tracking-wide text-[#51607A]">{c.l}</div>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div className="mx-auto mt-14 grid max-w-4xl gap-4">
            {diagnosis.map((d, i) => (
              <Reveal key={d.symptom} delay={i * 80}>
                <div className="grid items-center gap-4 rounded-2xl border border-[#E3E9F2] bg-white p-5 sm:grid-cols-[auto_1fr_auto_1fr] sm:p-6">
                  <span className="inline-grid h-11 w-11 place-items-center rounded-xl bg-[#18C29C]/10 text-[#0B8C73]">
                    <d.icon className="h-5 w-5" strokeWidth={1.8} />
                  </span>
                  <div>
                    <p className="text-[10px] font-bold uppercase tracking-wider text-[#7A8AA3]">Sintoma</p>
                    <p className="mt-0.5 font-space text-base font-bold text-[#0F1B2D]">{d.symptom}</p>
                  </div>
                  <ArrowDown className="h-5 w-5 justify-self-start text-[#2E7DF6] sm:rotate-[-90deg] sm:justify-self-center" />
                  <div>
                    <p className="text-[10px] font-bold uppercase tracking-wider text-[#7A8AA3]">Causa provável</p>
                    <p className="mt-0.5 text-sm leading-relaxed text-[#51607A]">{d.cause}</p>
                  </div>
                </div>
              </Reveal>
            ))}
          ...
          <div className="mt-14 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
            {equipment.map((e, i) => (
              <Reveal key={e.title} delay={(i % 4) * 80}>
                <div className="group h-full rounded-2xl border border-[#E3E9F2] bg-white p-6 transition-all duration-300 hover:-translate-y-1 hover:border-[#2E7DF6]/30">
                  <span className="inline-grid h-12 w-12 place-items-center rounded-xl bg-[#EAF1FE] text-[#2E7DF6] ring-1 ring-[#2E7DF6]/15 transition-colors duration-300 group-hover:bg-[#2E7DF6] group-hover:text-white">
                    <e.icon className="h-6 w-6" strokeWidth={1.7} />
                  </span>
                  <h3 className="mt-5 font-space text-base font-bold leading-snug text-[#0F1B2D]">{e.title}</h3>
                  <p className="mt-2 text-sm leading-relaxed text-[#51607A]">{e.desc}</p>
                </div>
              </Reveal>
            ))}
          </div>
      ...
      {/* Guarantee + CTA */}
      <section className="border-t border-[#E3E9F2] bg-[#F4F7FB] py-24">
        <div className="mx-auto max-w-7xl px-6">
