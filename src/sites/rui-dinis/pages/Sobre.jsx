...
export default function Sobre() {
  ...
  return (
    <>
          ...
          <Reveal>
            <div className="relative overflow-hidden rounded-xl bg-[#10161D] p-10">
              ...
              <div className="relative">
                <Quote className="h-10 w-10 text-[#2FA39B]" />
                ...
                <div className="mt-8 flex items-center gap-4 border-t border-white/10 pt-6">
                  <span className="grid h-12 w-12 place-items-center rounded-md bg-[#1A56DB] font-plexmono text-base font-semibold text-white">
                    RD
                  </span>
                  <div>
          ...
          <div>
            <Reveal>
              <p className="text-pretty text-lg leading-relaxed text-[#4C5A68]">
                A <span className="font-semibold text-[#10161D]">Rui Dinis — Reparações e Acessórios
            ...
            <Reveal delay={180}>
              <div className="mt-9 grid grid-cols-2 gap-px overflow-hidden rounded-lg border border-[#10161D]/10 bg-[#10161D]/10 sm:grid-cols-4">
                {stats.map((s) => (
          ...
          <div className="mt-14 grid gap-6 sm:grid-cols-2">
            {positioning.map((p, i) => (
              <Reveal key={p.title} delay={(i % 2) * 90}>
                <div className="flex h-full items-start gap-5 rounded-xl border border-[#10161D]/10 bg-white p-8">
                  <span className="inline-grid h-12 w-12 shrink-0 place-items-center rounded-md bg-[#10161D] text-[#2FA39B]">
                    <p.icon className="h-6 w-6" strokeWidth={1.6} />
                  </span>
                  <div>
                    <h3 className="font-plex text-xl font-bold text-[#10161D]">{p.title}</h3>
        ...
        <div className="mx-auto grid max-w-7xl items-center gap-14 px-6 lg:grid-cols-2">
            ...
            <div className="mt-8 grid gap-4 sm:grid-cols-2">
              {process.map((p, i) => (
                <Reveal key={p.step} delay={i * 70}>
                  <div className="rounded-xl border border-[#10161D]/10 bg-[#F4F6F9] p-5">
                    <span className="font-plexmono text-lg font-semibold text-[#1A56DB]">{p.step}</span>
                    <h3 className="mt-1 font-plex text-base font-bold text-[#10161D]">{p.title}</h3>
          ...
          <Reveal className="order-1 lg:order-2">
            <div className="relative overflow-hidden rounded-xl border border-[#10161D]/10">
              ...
              />
              <div className="absolute inset-0 bg-gradient-to-t from-[#10161D]/75 via-transparent to-transparent" />
      ...
      {/* CTA */}
      <section className="relative overflow-hidden bg-[#10161D] py-20">
        <GridField className="absolute inset-0" tone="#FFFFFF" opacity={0.05} />
        <div className="pointer-events-none absolute inset-0">
          <div className="absolute -right-16 top-0 h-72 w-72 rounded-full bg-[#1A56DB]/25 blur-3xl" />
        </div>
        <div className="relative mx-auto flex max-w-4xl flex-col items-center gap-7 px-6 text-center">
          ...
          <Reveal delay={120}>
            <Link
              ...
              className="inline-flex items-center gap-2 rounded-md bg-[#1A56DB] px-8 py-4 font-plex text-base font-bold text-white transition-colors hover:bg-[#1648b8] cursor-pointer"
            >
