...
export default function Solucoes() {
  ...
  return (
    <>
          ...
          <div className="space-y-5">
            {solutions.map((s, i) => (
              <Reveal key={s.title} delay={(i % 2) * 80}>
                <div className="group grid items-start gap-6 rounded-xl border border-[#16182B]/10 bg-[#F4F6F9] p-7 transition-all duration-300 hover:border-[#2E3192]/40 hover:bg-white hover:shadow-xl hover:shadow-[#16182B]/5 sm:grid-cols-[auto_1fr_auto] sm:p-8">
                  <span className="inline-grid h-16 w-16 place-items-center rounded-lg bg-[#16182B] text-white transition-colors duration-300 group-hover:bg-[#2E3192]">
                    <s.icon className="h-8 w-8" strokeWidth={1.5} />
                  </span>
                  <div>
                    <div className="flex flex-wrap items-center gap-3">
                      <h2 className="font-plex text-2xl font-bold text-[#16182B]">{s.title}</h2>
                      <span className="rounded border border-[#2E3192]/30 bg-[#2E3192]/5 px-2 py-0.5 font-plexmono text-[10px] font-semibold uppercase tracking-[0.14em] text-[#2E3192]">
                        {s.code}
                      </span>
                    </div>
                    ...
                    <div className="mt-4 flex flex-wrap gap-1.5">
                      {s.tags.map((t) => (
                        <span
                          key={t}
                          className="rounded bg-white px-2.5 py-1 font-plexmono text-[10px] uppercase tracking-[0.1em] text-[#4C5A68] ring-1 ring-[#16182B]/10"
                        >
                          {t}
                        </span>
                      ))}
                  ...
                  <Link
                    ...
                    className="inline-grid h-11 w-11 shrink-0 place-items-center self-center rounded-md border border-[#16182B]/15 text-[#16182B] transition-colors duration-200 hover:border-[#2E3192] hover:bg-[#2E3192] hover:text-white cursor-pointer"
                  >
      ...
      <section className="bg-[#F4F6F9] py-20">
        <div className="mx-auto max-w-7xl px-6">
          <Reveal className="max-w-2xl">
            <span className="font-plexmono text-xs font-semibold uppercase tracking-[0.18em] text-[#9A6206]">
          ...
          <div className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
            {consumables.map((c, i) => (
              <Reveal key={c.title} delay={(i % 4) * 70}>
                <div className="h-full rounded-xl border border-[#16182B]/10 bg-white p-7">
                  <span className="inline-grid h-12 w-12 place-items-center rounded-md bg-[#E0A126]/15 text-[#9A6206]">
                    <c.icon className="h-6 w-6" strokeWidth={1.6} />
                  </span>
                  <h3 className="mt-5 font-plex text-lg font-bold text-[#16182B]">{c.title}</h3>
        ...
        <div className="mx-auto grid max-w-7xl items-center gap-14 px-6 lg:grid-cols-2">
          <Reveal>
            <div className="relative overflow-hidden rounded-xl border border-[#16182B]/10">
              ...
              />
              <div className="absolute inset-0 bg-gradient-to-t from-[#16182B]/70 via-transparent to-transparent" />
              <span className="absolute bottom-4 left-4 rounded bg-[#16182B]/80 px-3 py-1.5 font-plexmono text-[11px] uppercase tracking-[0.16em] text-[#E0A126] backdrop-blur">
            ...
            <div className="mt-8 space-y-4">
              {process.map((p, i) => (
                <Reveal key={p.step} delay={i * 80}>
                  <div className="flex items-start gap-4 rounded-xl border border-[#16182B]/10 bg-[#F4F6F9] p-5">
                    <span className="font-plexmono text-xl font-semibold text-[#2E3192]">{p.step}</span>
                    <div>
                      <h3 className="font-plex text-lg font-bold text-[#16182B]">{p.title}</h3>
      ...
      {/* Sectors quick band */}
      <section className="relative overflow-hidden bg-[#16182B] py-20 text-[#F4F6F9]">
            ...
            {sectors.map((s, i) => (
              <Reveal key={s.title} delay={(i % 5) * 50}>
                <span className="inline-flex items-center gap-2 rounded-md border border-white/10 bg-white/[0.04] px-4 py-2.5 font-plex text-sm font-semibold text-[#E4EAF1]">
                  <s.icon className="h-4 w-4 text-[#E0A126]" strokeWidth={1.6} /> {s.title}
                </span>
              </Reveal>
            ))}
          </div>
      ...
      <section className="bg-[#F4F6F9] py-20">
        <div className="mx-auto max-w-7xl px-6">
          <Reveal>
            <div className="relative overflow-hidden rounded-xl bg-[#2E3192] px-8 py-12 sm:px-14 sm:py-16">
              <GridField className="absolute inset-0" tone="#FFFFFF" opacity={0.09} />
              <div className="pointer-events-none absolute -right-16 -top-16 h-64 w-64 rounded-full bg-[#E0A126]/30 blur-3xl" />
              <div className="relative flex flex-col items-start gap-8 lg:flex-row lg:items-center lg:justify-between">
                ...
                <Link
                  to={siteLink(slug, 'contacto')}
                  className="inline-flex shrink-0 items-center gap-2 rounded-md bg-white px-7 py-4 font-plex text-base font-bold text-[#2E3192] transition-transform duration-200 hover:scale-[1.02] cursor-pointer"
                >
      ...
      <section className="bg-white py-20">
        <div className="mx-auto flex max-w-3xl flex-col items-center gap-7 px-6 text-center">
          <Reveal>
            <h2 className="font-plex text-4xl font-bold leading-tight text-[#16182B]">
          ...
          <Reveal delay={120}>
            <Link
              to={siteLink(slug, 'contacto')}
              className="inline-flex items-center gap-2 rounded-md bg-[#16182B] px-8 py-4 font-plex text-base font-bold text-white transition-colors hover:bg-[#2E3192] cursor-pointer"
            >
